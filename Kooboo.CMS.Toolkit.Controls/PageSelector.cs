...
namespace Kooboo.CMS.Toolkit.Controls
{
    public class PageSelector : Form.Html.Controls.DropDownList
    {
        ...
        protected override string RenderInput(IColumn column)
        {
            ...
            {{                            
                <option value=""@item.UUID"" @((Model.{0} != null && Model.{0}.ToString().ToLower() == @item.UUID.ToLower()) || (Model.{0} == null && dropDownDefault_{0}.ToLower() == @item.UUID.ToLower()) ? ""selected"" : """")>@item.FriendlyName</option>
            }}
